...
{   
    ...
    Model::Model(std::string const& path) 
     { 
        ...
        HZ_CORE_INFO("go into root node");
        ...
        HZ_CORE_INFO("scene traverse finished");
    ...
    void Model::processNode (const aiScene* scene, aiNode* ai_node, Ref<Object> scene_node )
    {
        ...
        HZ_CORE_INFO("modelloader: find {0} meshes in the node: {1}", ai_node->mNumMeshes, name);
        for (uint32_t i = 0; i < ai_node->mNumMeshes; i++)
        {
            ...
            HZ_CORE_INFO("process mesh");
            meshObject->mesh = processMesh(ai_mesh, scene);

            HZ_CORE_INFO("process material");
    ...
    void Model::processBones(const aiScene* scene, aiMesh* mesh,  std::vector<Vertex>& vertices)
    {
        ...
        HZ_CORE_INFO("Modelloading: {0} bound bones detected for the mesh", mesh->mNumBones);
        for (int boneIndex = 0; boneIndex < mesh->mNumBones; ++boneIndex)
        {
            int boneID = -1;
            std::string boneName = mesh->mBones[boneIndex]->mName.C_Str();
            //HZ_CORE_INFO("bone name detected {0}", boneName);
            ...
            if (this->bindPoseBoneMap.find(boneName) == bindPoseBoneMap.end())  //not registered yet
            {
                ...
                HZ_CORE_INFO("bound bone registered, name:{0}, id:{1}", boneName, boneID);

            }
            else   //already registered
            {
                boneID = bindPoseBoneMap[boneName].boneIndex;
                HZ_CORE_INFO("bone:{0} already registered", boneName);
            }
            ...
            //HZ_CORE_INFO("Modelloading: {0} bounded vertices detected for the bone", numWeights);
            ...
            for (int weightIndex = 0; weightIndex < numWeights; ++weightIndex)
            {
                ...
                { 
                       ...
                       if (vertices[vertexId].BoneWeights[i] == 0) //the first empty slot
                    {
                       ...
                       // HZ_CORE_INFO("modelloader weight:{0},id:{1}, vertex {2}", weight,boneID, vertexId);
                        break;
                    }
                } 
            }
        ...
        auto loadTexture = [&](aiTextureType type, TextureType textureType) {
            ...
            if (ai_material->GetTexture(type, 0, &str) == AI_SUCCESS)
            {
                ...
                if (!skip)
                {
                    ...
                    HZ_CORE_WARN("Modelloading: Texture type: {0} is added to material ", (int)textureType);
                }
    ...
    void Model::processAnimation(const aiScene* scene)
    {
        ...
        HZ_CORE_INFO("Modelloading: Animation num:{0}",scene-> mNumAnimations);
        ...
        HZ_CORE_INFO("animationclip created, duration:{0}, fps:{1}", duration, FPS);
        ...
        HZ_CORE_INFO("""animationclip:{0} channels ", ai_animation->mNumChannels);
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            //HZ_CORE_INFO("Modelloading: animated bone name found:{0}", boneName);
            ...
            if (node)
                ...
                HZ_CORE_ERROR("Modelloading: animted bone name not found in the scene:{0}", boneName);


            //retreive properties in the channel
            //HZ_CORE_WARN("Modelloading: mNumPositionKeysfound:{0}", channel->mNumPositionKeys);
            ...
            HZ_CORE_INFO("animclip: add animated keybone:{0} ", boneName);
        }
