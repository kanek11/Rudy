...
namespace Hazel
{
	...
	void AnimationClip::CalculateKeyframe(float time)
	{
		...
		//HZ_CORE_INFO("animationclip: the floor frame index is {0}", floor_frameIndex);
	...
	{ 
		...
		if (_currentKeyBones.find(name) == _currentKeyBones.end())
			HZ_CORE_INFO("animclip: bone {0} is not found as keyed properties", name);
	...
	std::vector<glm::mat4> Animator::GetBoneTransforms(float time)
	{
		...
		{
			HZ_CORE_ERROR("no animation found");
		}
		...
		for (const auto& bonePair : model->bindPoseBoneMap) {


			const std::string& boneName = bonePair.first;
			auto bone = bonePair.second;  

			glm::mat4 modelToLocal = bone.bindPoseTransform;

			//the default
			glm::mat4 animLocalToGlobal = glm::inverse(bone.bindPoseTransform);
			...
			m_BoneTransforms[bone.boneIndex] = animLocalToGlobal * modelToLocal;
			//m_BoneTransforms[bone.boneIndex] = glm::mat4(1.0f);
			//HZ_CORE_INFO("animatior: set bone:{0}", bone.boneIndex);
	...
	void AnimationClip::printHierarchy(const Ref<Transform> node)
	{
		...
		HZ_CORE_INFO("animationclip print: node name:{0}", node->gameObject->name);
