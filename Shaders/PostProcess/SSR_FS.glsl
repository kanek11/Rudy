...
uniform sampler2D u_litPassTexture;
...
{ 
    ...
    //vec3 screenColor = texture(u_litPassTexture, TexCoords).rgb;
    vec3 normal = texture(gWorldNormal, TexCoords).rgb * 2.0 - 1.0;
    float ViewDepth = texture(gViewDepth, TexCoords).r;
    ...
    if (ViewDepth < 1 && dot(-viewDir, normal) > 0.0 )
    {  
        ...
        for (int i = 0; i < 512; ++i) // march 32 steps for example
        {
            ...
            float currentFragDepth = texture(gViewDepth, sampleUV).r;
            ...
            //float sampleDepth = texture(gViewDepth, sampleUV).r; 

            if (abs(currentDepth - FragDepth) < 0.01) // If the depths are close, a reflection is found
            {
                indirColor = texture(u_litPassTexture, sampleUV).rgb;
                hit = 1;
                break;
            }
        }
