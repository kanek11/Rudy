...
float sample_field(int type, float _x, float _y)
{
    ...
    int   height  = int(u_height);
    float dx      = u_dx;
    float dx_inv  = 1.0f / dx;
    float dx_half = 0.5f * dx;

    // cap to lower boundary
    ...
    x0 = min(int(floor((x - x_offset) * dx_inv)), width - 1);
    x1 = min(x0 + 1, width - 1);
    y0 = min(int(floor((y - y_offset) * dx_inv)), height - 1);
...
void main()
{
    ...
    {
        // recover velocity
        float u_vel = (vel_u_post.vel_u_post[center] + vel_u_post.vel_u_post[right]) * 0.5f;
        float v_vel = (vel_v_post.vel_v_post[center] + vel_v_post.vel_v_post[top]) * 0.5f;

        // backtrace by simple euler
        float x = i * u_dx - u_vel * u_dt + dx_half;
        float y = j * u_dx - v_vel * u_dt + dx_half;

        // sample velocity by bilinear interpolation
        mass_post.mass_post[center] = sample_field(CELL, x, y);
    }
}
