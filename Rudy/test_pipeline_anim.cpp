...
//pose it to the first frame; 
   ...
   // lightingPassMaterial->SetTexture(TextureType::brdfLUT, brdfLUTMap);
    ...
    ////RD_CORE_ERROR("size of lightingpass material textures: {0}", lightingPassMaterial->GetTextures().size());
    ...
    // RD_CORE_INFO("lighting pass quad material size: {0}", lightingPassMaterial->GetTextures().size());
    //  
    ...
    while (!Renderer::ShouldClose())
    {
        ...
        //======= lighting pass: render the quad;
        {
            ...
            // RD_CORE_INFO("camera pos{0}", main_camera->GetPosition());
      ...
      //ssrShader->SetVec3("u_CameraPos", main_camera->GetPosition()); 
      ////RD_CORE_INFO("cameraPos: {0}", main_camera->GetPosition());
      //ssrQuad.Draw();  
      //ssrFBO->Unbind();
