...
Model::Model(std::string const& path)
{
    ...
    const aiScene* scene = importer.ReadFile(path, aiProcess_GlobalScale | aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace);
    ...
    this->rootNode = Object::Create();
    processNode(scene, scene->mRootNode, this->rootNode);
...
void Model::processNode(const aiScene* scene, aiNode* ai_node, SharedPtr<Object> scene_node)
{
    ...
    scene_node->name = ai_node->mName.data;
...
void Model::processBones(const aiScene* scene, aiMesh* mesh, std::vector<Vertex>& vertices)
{
    ...
    int boneCounter = boneMap.size(); // the current number of bones in the map;
    ...
    for (unsigned int boneIndex = 0; boneIndex < mesh->mNumBones; ++boneIndex)
    {
        ...
        if (this->boneMap.find(boneName) == boneMap.end()) // not registered yet
        {
            auto bindPoseTransform = AssimpGLMHelpers::ConvertMatrixToGLMFormat(mesh->mBones[boneIndex]->mOffsetMatrix);

            boneMap[boneName]     = boneCounter;
            bindPose[boneCounter] = bindPoseTransform; // the rest pose of the bone;]
...
void Model::processAnimation(const aiScene* scene)
{
    ...
    RD_CORE_INFO(""
                 "animationclip:{0} channels ",
                 ai_animation->mNumChannels);
...
SharedPtr<Object> Model::findNodeRecursive(SharedPtr<USceneComponent> node, std::string const& name)
{
    if (node == nullptr)
    {
        return nullptr;
