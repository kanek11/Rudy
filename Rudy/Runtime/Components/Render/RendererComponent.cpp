...
void StaticMeshRenderer::Draw(SharedPtr<UCameraComponent> cam, uint32_t count, SharedPtr<Material> mat)
{
    if (!hasMesh())
    {
...
void ParticleSpriteRenderer::Draw(SharedPtr<UCameraComponent> cam, uint32_t count, SharedPtr<Material> mat)
{
    // disable depth test
