...
void SSAO::Init()
{
    ...
    SSAOOnlyTex = Texture2D::CreateEmpty(TextureSpec {
        m_width, m_height, TextureInternalFormat::R32F, false, WrapMode::ClampToEdge, FilterMode::Nearest });

    SSAOBlurTexture = Texture2D::CreateEmpty(
    ...
    auto SSAONoiseTexture = Texture2D::CreateEmpty(TextureSpec {
        4, 4, TextureInternalFormat::RGB32F, false, WrapMode::Repeat, FilterMode::Nearest });
...
void SSAO::Render(SharedPtr<Camera> main_camera)
{
    SSAOShader->Bind();
