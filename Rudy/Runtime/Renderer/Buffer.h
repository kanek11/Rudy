...
class FRHIVertexBuffer : public virtual FRHIBuffer
{
public:
    static SharedPtr<FRHIVertexBuffer> Create();

    // uint32_t m_Stride;
    // layout info.
};

class FRHIIndexBuffer : public virtual FRHIBuffer
{
public:
    static SharedPtr<FRHIIndexBuffer> Create();
};

// for cumpute shader.
// we use "shader storage buffer" to store the data that is used by compute shader.
// in vulkun, it's also called "storage buffer".
class RHIStorageBuffer : public virtual FRHIBuffer
{
public:
    static SharedPtr<RHIStorageBuffer> Create();
    virtual void                       BindBase(uint32_t index) const = 0; // specify the binding point;
};

// for small, shared data among shaders.
class FRHIUniformBuffer : public virtual FRHIBuffer
{
public:
    static SharedPtr<FRHIUniformBuffer> Create();
    virtual void                        BindBase(uint32_t index) const = 0; // specify the binding point;
};
...
{
public:
    static SharedPtr<FRHIVertexArray> Create();
