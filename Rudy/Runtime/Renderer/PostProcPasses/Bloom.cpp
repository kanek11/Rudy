...
Bloom::Bloom(
    uint32_t m_width, uint32_t m_height, std::unordered_map<TexType, SharedPtr<UTexture>>& m_BloomInputs, std::unordered_map<TexType, SharedPtr<UTexture>>& m_BloomOutputs) :
    m_width(m_width),
    m_height(m_height),
...
void Bloom::Init()
{
    ...
    {
        RT_Horizonal.push_back(NewObject<UTexture2D>("", TextureConfig { mip_res.x, mip_res.y, TexFormat::RGBA32F, 1, false, WrapMode::CLAMP_TO_EDGE, FilterMode::LINEAR, FilterMode::LINEAR }));

        RT_Vertical.push_back(NewObject<UTexture2D>("", TextureConfig { mip_res.x, mip_res.y, TexFormat::RGBA32F, 1, false, WrapMode::CLAMP_TO_EDGE, FilterMode::LINEAR, FilterMode::LINEAR }));
...
void Bloom::Render()
{
    ...
    if (true)
    {
        ...
        for (int mip = 0; mip < nMips - 1; mip++)
        {
            ...
            // input
            RT_Horizonal[mip + 1]->BindToSlot(0);
            // output
            glBindImageTexture(0, RT_Vertical[mip + 1]->GetID(), 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32F);

            // dispatch
            glDispatchCompute(
                static_cast<uint32_t>(glm::ceil(float(mip_res.x) / 8)),
                static_cast<uint32_t>(glm::ceil(float(mip_res.y) / 8)),
                1);
            glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
        }
        ...
        glDispatchCompute(
            static_cast<uint32_t>(glm::ceil(float(m_width) / 8)),
            static_cast<uint32_t>(glm::ceil(float(m_height) / 8)),
            1);
        glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);
